...
function Strategy(options, verify) {
  ...
  this._redirectQueries = options.redirectQueries || [];
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if(this._redirectQueries && Array.isArray(this._redirectQueries) && req && req.query){
      for(var i = 0; i < this._redirectQueries.length ; i++){
        var item = this._redirectQueries[i];
        var value = req.query[item.key];
        if(value) {
